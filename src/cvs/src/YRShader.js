	...
	constructor() {
		...
		YRShader.FlyLine = {
			...
			vertexShader: `
				...
				{
					pos=vec3(position);
					if(pos.x>time-size&&pos.x<time)
					{
						gl_PointSize=10.0*(pos.x-time+size)/size;
					}
			...
			fragmentShader: `
				...
				void main()
				{
					...
					if(abs(pos.x - end) < 0.2 || abs(pos.x - time) < 0.2){
						if(d > 0.5){
							discard;
						}
					}
		...
		YRShader.Open = {
			...
			fragmentShader: `
				...
				void main()
				{
					if(vUv.y>0.5-time&&vUv.y<0.5+time)
					{
		...
		YRShader.GlitchNormal = {
			...
			fragmentShader: `
				...
				void main()
				{
					...
					uvcoord.x+=(-1.+2.*random(time+999.))/25.;
					uvcoord.y+=(-1.+2.*random(time+999.))/25.;
					color=texture2D(img0,uvcoord).rgb;

					for(int i=0;i<3;i+=1)
					{
						sliceY=random(time+float(i));
						sliceH=random(time+float(i))*0.25;
						sliceOffestX=random(time+float(i)+9999.0)/5.0;
						
						if(vUv.y>sliceY&&vUv.y<sliceY+sliceH)
						{
							uvcoord.x-=sliceOffestX;
							float rdcolor=random(time+555.);
							if(rdcolor<0.33)
							{
								color.r=texture2D(img0,uvcoord).r;
							}
							else if(rdcolor<0.66)
							{
								color.g=texture2D(img0,uvcoord).g;
							}
							else if(rdcolor<0.99)
							{
								color.b=texture2D(img0,uvcoord).b;
							}
					...
					// else
					// {
					// 	color=texture2D(img0,vec2(vUv.x-0.2,vUv.y));
					// }
					
					// if(vUv.y>0.4&&vUv.y<0.6)
					// {
					// 	colorR=texture2D(img0,vec2(vUv.x-0.1,vUv.y)).r;
					// }
					// else
					// {
					// 	colorR=texture2D(img0,vec2(vUv.x-0.0,vUv.y)).r;
					// }
